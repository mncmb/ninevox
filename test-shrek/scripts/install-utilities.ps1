...
choco install -y --limit-output --no-progress NotepadPlusPlus processhacker git 7zip chocolateygui

# This repo often causes failures due to incorrect checksums, so we ignore them for Chrome
choco install -y --limit-output --no-progress --ignore-checksums GoogleChrome firefox
