...
# Also installs Sysmon and Florian Roth sysmon config
...
# Download Florian Roth Sysmon config
Write-Host "$('[{0:HH:mm}]' -f (Get-Date)) Downloading Florian Roth's Sysmon config..."
(New-Object System.Net.WebClient).DownloadFile('https://raw.githubusercontent.com/Neo23x0/sysmon-config/master/sysmonconfig-export.xml', "$sysmonConfigPath")
